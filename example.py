...
async def main():
    # ISP
    name, birthdate, phone, ISP = "홍길동", "0102034", "01012345678", "KT"
    ...
    initResult = await verification.init_session()
    if not initResult['Success']:
        return {"Success": False, "Message": initResult['Message']}

    captchaResult = await verification.get_captcha()
    ...
    sendResult = await verification.send_SMS_verify(name, birthdate, phone, captchaCode)
    if not sendResult['Success']:
        return {"Success": False, "Message": sendResult['Message']}
    
    smsCode = input("SMS로 전송된 코드를 입력해주세요: ")
    checkResult = await verification.check_SMS_verify(smsCode)
